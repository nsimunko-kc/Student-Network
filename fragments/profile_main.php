...
if($result->num_rows == 0) {
    header('Location: exceptions/user_not_found.php');
}
...
$postCount = mysqli_fetch_assoc($db->query('SELECT COUNT(*) AS post_count FROM posts WHERE user_id = ' . $pid));

$commentCount = mysqli_fetch_assoc($db->query('SELECT COUNT(*) AS comm_count FROM comments WHERE user_id = ' . $pid));
?>
<div class="container">
    <aside class="left-aside">
        <div>
            <img src="images/profile_images/<?= ($user['profile_img_path'] == null ? 'default_profile.jpg' : $user['profile_img_path']) ?>" alt="Profile image">
            <a href="profile.php?pid=<?=$pid?>"><?= $user['username'] ?></a>
    ...
    <section class="post-section">
        <?php if($pid == $_SESSION['user_id']) {
            include("fragments/new_post_div.php");
        } ?>
        <div class="post-timeline-div">
            <?php
            $result = $postHandler->getPosts('profile', $pid);
